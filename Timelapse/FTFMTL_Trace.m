...
function out=FTFMTL_Trace(varargin)
...
if cond2 == 1
%     imshow(FinalImage(:,:,NumberImages),[0 0.2*max(max(FinalImage(:,:,NumberImages)))])
    imshow(StackImage,[0 0.5*max(StackImage(:))])
...
for i = [1:NumberImages]
    ...
    retrace = input('Do you want to retrace cell boundary? 1-yes 0-no: ');
    if retrace == 1
        figure, imshow(cellimg,[])
